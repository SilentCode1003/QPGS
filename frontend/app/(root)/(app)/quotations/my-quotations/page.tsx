...
async function fetchMyQuotations(userId: number) {
  const res = await api.get<QuotationsResponse>(
    `/quotations?created_by=${encodeURIComponent(userId)}`,
  )
...
export default function MyQuotations() {
  ...
  const quotations = useQuery({
    queryKey: [`quotations?created_by=${user.data.id}`],
    queryFn: () => fetchMyQuotations(user.data.id),
    staleTime: 0,
    initialData: [],
  })
